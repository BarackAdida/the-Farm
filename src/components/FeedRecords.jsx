...
const FeedRecords = () => {
  ...
  const fetchFeeds = async () => {
    const res = await fetch("https://the-farm-backend.onrender.com/feeds");
  ...
  const useFeed = async (id) => {
    await fetch(`https://the-farm-backend.onrender.com/feeds/use/${id}`, { method: "POST" });
    fetchFeeds();
  };

  const deleteFeed = async (id) => {
    await fetch(`https://the-farm-backend.onrender.com/feeds/${id}`, { method: "DELETE" });
    fetchFeeds();
  };

  const updateFeed = async (e) => {
    e.preventDefault();
    await fetch(`https://the-farm-backend.onrender.com/feeds/${editFeedData.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
